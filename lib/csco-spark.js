...
module.exports = function(params) {
  ...
  handler.authenticate = (auth) => {
    ...
    return _handleReq(oauthFlow.loginReq()).then((html) => {
      // Parse Login Code from HTML Body (authService)
      var loginCode = oauthFlow.parseLoginCode(html);
      // Get Authorization REQ Options using Login Code (authService)
      // Make Authorize Request (_handleReq)
      return _handleReq(oauthFlow.authorize(loginCode));
    }).then((resp) => {
      ...
      if(authCode === 'Authentication Denied') {
        ...
        return _handleReq(oauthFlow.accessToken({
          code: authCode
