  ...
  var getPath = (path) => {
    ...
    return (
      ...
      (item==='rooms' || item==='webhooks') ?
        `/${item}?max=${max}` :
      (item==='memberships') ?
        `/${item}?${path.roomId ? `roomId=${path.roomId}` :
        path.personId ? `personId=${path.personId}` :
        path.email ? `personEmail=${path.email}`: undefined}
        &max=${max}` :
      (items==='people') ?
        `/${item}?${args.email ? `email=${path.email}` :
        path.displayName ? `displayName=${path.displayName}` :
