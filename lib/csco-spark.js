...
module.exports = (params) => {
  ...
  var getPath = (path) => {
    ...
    return (
      (item==='messages') ?
        `/${item}?roomId=${path.roomId}&max=${max}` :
      (item==='rooms' || item==='webhooks') ?
        `/${item}?max=${max}` :
      (item==='memberships' && path.roomId) ?
        `/${item}?roomId=${path.roomId}&max=${max}` :
      (item==='memberships' && path.personId) ?
        `/${item}?personId=${path.personId}` :
      (item==='memberships' && path.email) ?
        `/${item}?email=${path.email}` :
      (item==='memberships' && path.displayName) ?
        `/${item}?displayName=${path.displayName}` :
      (item==='people' && path.email) ?
        `/${item}?email=${path.email}` :
      (item==='people' && path.displayName) ?
        `/${item}?displayName=${path.displayName}` :
      (item==='people' && path.personId) ?
        `/${item}?personId=${path.personId}` :
      undefined
    );
  };
  ...
  handler.listItemEvt = (opts) => {
    ...
    _handleReq({
      ...
      if(resp.headers) {
        ...
        handler.emit(item, JSON.parse(resp).items);
      }
    });
    return handler;
  };
